...
<div class="row">
    <div class="col-12  mb-15">
        <div class="card mb-2">
            <div class="card-header text-center bg-success text-white">Category Update From </div>
        </div>
    ...
    <div class="col-12">
        <form  action="{{ url('/admin/categories/update/' .$categories->id .'/' .$categories->name) }}" method="POST" enctype="multipart/form-data">
            @csrf
            @method("PATCH")
            ...
            <div class="row">
                <div class="col-md-6">
                    <div class="mb-3">
                        ...
                        <input type="text" name="name" class="form-control" placeholder="Name" value="{{ $categories->name }}" required>
                    </div>
                    <div class="mb-3">
                        <label for="title" class="form-label">Description</label>
                        <textarea cols="30" rows="10" name="title" class="form-control" id="title" required>{{ $categories->title }}</textarea>
                    </div>
