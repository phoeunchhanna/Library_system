...
@section('content')
    <div class="container-fluid">
        ...
        <div class="row">
            ...
            <div class="col-12">
                <div class="card mb-2 ">
                    <div class="card-header text-center bg-success text-white">Book Application </div>
                </div>
                <div class="row mb-5">
                    ...
                    <div class="col-md-6 text-right">
                        ...
                        <a href="/admin/books/create" class="btn btn-success btn-sm ml-2 text-sm-center p-2">
                            Add New courses
                        </a>
                    </div>
                </div>
                ...
                <table class="data-table table stripe hover nowrap dataTable collapsed" id="DataTables_Table_0" role="grid" aria-describedby="DataTables_Table_0_info">
                    <thead>
                        <tr role="row">
                            ...
                            <th class="table-plus datatable-nosort sorting_asc" rowspan="1" colspan="1" aria-label="Name">Category</th>
                            <th class="table-plus datatable-nosort sorting_asc" rowspan="1" colspan="1" aria-label="Name">Author</th>
                            <th class="table-plus datatable-nosort sorting_asc" rowspan="1" colspan="1" aria-label="Name">Personalize</th>
                    ...
                    <tbody>
                        ...
                        <tr role="row" class="odd">
                            ...
                            <td class="table-plus sorting_1" tabindex="0">{{ $item->Category->name }}</td>
                            <td class="table-plus sorting_1" tabindex="0">{{ $item->author->name }}</td>
                            <td class="table-plus sorting_1" tabindex="0">{{ $item->personalize }}</td>                          
                            <td class="table-plus sorting_1" tabindex="0">
                                @if($item->image)
                                    <img src="{{ asset('photo/book/' . $item->image) }}" 
                            ...
                            <td class="text-center">
                               <div class="d-flex align-items-center justify-between p-0">
                                ...
                                <form class="d-inline"  method="POST" action="{{ url('/admin/books/delete/'. $item->id) }}"
                ...
                {{-- <div class="d-flex justify-content-center ">
                    <nav aria-label="Page navigation example ">
                        <ul class="pagination ">
                            <li class="page-item {{ $athorDetail->previousPageUrl() ? '' : 'disabled' }}">
                                <a class="page-link" href="{{ $athorDetail->previousPageUrl() ?? '#' }}" aria-label="Previous">
                                    <span aria-hidden="true">&laquo;</span>
                                </a>
                            </li>
                            @for($i = 1; $i <= $books->lastPage(); $i++)
                                <li class="page-item {{ $i === $athorDetail->currentPage() ? 'active' : '' }}">
                                    <a class="page-link" href="{{ $athorDetail->url($i) }}">{{ $i }}</a>
                                </li>
                            @endfor
                            <li class="page-item {{ $athorDetail->nextPageUrl() ? '' : 'disabled' }}">
                                <a class="page-link" href="{{ $athorDetail->nextPageUrl() ?? '#' }}" aria-label="Next">
